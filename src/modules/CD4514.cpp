//----------------------------------------------------------------------------
//	Lunetta Modula Plugin for VCV Rack by Count Modula - CD4514
...
#define MODULE_NAME CD4514

struct CD4514 : Module {
	enum ParamIds {
		NUM_PARAMS
	};
	enum InputIds {
	...
	CD4514() {
		...
		inputInfos[STROBE_INPUT]->description = "Loads the decoder with the logic values present at the A-D inputs and sets the appropriate output high";
		inputInfos[INHIBIT_INPUT]->description = "Forces all outputs low";

		for (int b = 0; b < 16; b++) {
			configOutput(S_OUTPUTS + b, rack::string::f("S%d", b));
			outputInfos[S_OUTPUTS + b]->description = "Active high (low when inhibited)";
		}
	...
	void onReset() override {
		...
		for(int i = 0; i < 16; i++) {
			outputs[S_OUTPUTS + i].setVoltage(0.0f);
			lights[S_LIGHTS + i].setBrightness(0.0f);
		}
	...
	void process(const ProcessArgs &args) override {
		...
		if (prevCount != countToUse || prevInh != inh) {
			...
			if (prevCount >=0) {
				outputs[S_OUTPUTS + prevCount].setVoltage(0.0f);
				lights[S_LIGHTS + prevCount].setBrightness(0.0f);
			}
...
struct CD4514Widget : ModuleWidget {
	CD4514Widget(CD4514 *module) {
		setModule(module);
		setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/CD4514.svg")));

		// screws
		#include "../components/stdScrews.hpp"	

		// inputs
		addInput(createInputCentered<LunettaModulaLogicInputJack>(Vec(STD_COLUMN_POSITIONS[STD_COL1], STD_HALF_ROWS8(STD_ROW1)), module, CD4514::A_INPUT));
		addInput(createInputCentered<LunettaModulaLogicInputJack>(Vec(STD_COLUMN_POSITIONS[STD_COL1], STD_HALF_ROWS8(STD_ROW2)), module, CD4514::B_INPUT));
		addInput(createInputCentered<LunettaModulaLogicInputJack>(Vec(STD_COLUMN_POSITIONS[STD_COL1], STD_HALF_ROWS8(STD_ROW3)), module, CD4514::C_INPUT));
		addInput(createInputCentered<LunettaModulaLogicInputJack>(Vec(STD_COLUMN_POSITIONS[STD_COL1], STD_HALF_ROWS8(STD_ROW4)), module, CD4514::D_INPUT));
		addInput(createInputCentered<LunettaModulaLogicInputJack>(Vec(STD_COLUMN_POSITIONS[STD_COL1], STD_HALF_ROWS8(STD_ROW6)), module, CD4514::STROBE_INPUT));
		addInput(createInputCentered<LunettaModulaLogicInputJack>(Vec(STD_COLUMN_POSITIONS[STD_COL1], STD_HALF_ROWS8(STD_ROW7)), module, CD4514::INHIBIT_INPUT));

		// decoded outputs
		int outputId = CD4514::S_OUTPUTS;
		int lightId = CD4514::S_LIGHTS;
		int col = STD_COL3;
		for (int i = 0; i < 2; i++) {
			for (int b = 0; b < 8; b++) {
	...
	void appendContextMenu(Menu *menu) override {
		CD4514 *module = dynamic_cast<CD4514*>(this->module);
		assert(module);
...
Model *modelCD4514 = createModel<CD4514, CD4514Widget>("CD4514");
